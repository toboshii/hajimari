    ...
    const handleSubmit = () => {
        ...
        if (matches) {
            ...
            if (provider && queryText) {
                window.location.assign(provider.searchUrl.replaceAll('{query}', queryText))
            } else if (provider) {
            ...
            window.location.assign(provider.searchUrl.replaceAll('{query}', query))
        }
    }
...
<section id="search">
    <form on:submit|preventDefault={handleSubmit}>
        ...
        <input bind:value={query} type="text" id="keywords" size="50" spellcheck="false" autofocus={true} />
    </form>
</section>
