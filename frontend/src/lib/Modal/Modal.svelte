<script lang="ts">
  ...
  async function handleSave(): Promise<void> {
    ...
    if (response.status >= 200 && response.status <= 299) {
      ...
      alert(`Error: ${jsonResponse.status}\n\nReason: ${jsonResponse.error}`);
    }
  }
...
>
  ...
  {#if showHelp}
    <section id="providers" transition:scale>
      <h2>Search</h2>
      <span class="help"
        >The default provider can be changed using <pre>defaultSearchProvider</pre>
        in the configuration</span
      >
      <table>
        <tr>
          <th>Provider</th>
  ...
  <section id="themes">
    ...
    <ThemeSwitcher
      bind:settings
      on:updateEditor={handleUpdateEditor}
      {showHelp}
    />
  </section>
  ...
  <section id="config">
    ...
    {#if showHelp}
      <span class="help"
        ><b>Ctrl-F/G:</b> Find <b>Alt-G:</b> Goto line <b>Alt-R:</b> Fold group</span
      >
    {/if}
    <div class="editor">
